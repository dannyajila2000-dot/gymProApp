...
export default function Login() {
  ...
  async function enviar() {
    setError(null);
    if (!codigoGimnasio || !email || !password) {
      setError('Completa todos los campos');
      return;
    }
    setCargando(true);
    try {
      await iniciarSesion({ codigoGimnasio: codigoGimnasio.trim(), email: email.trim(), password });
      router.replace('/(tabs)');
    } catch (e) {
  ...
  return (
    <KeyboardAvoidingView
      ...
      <ScrollView contentContainerStyle={styles.contenedor} keyboardShouldPersistTaps="handled">
        <Text style={[styles.titulo, { color: colors.text }]}>Bienvenido</Text>
        <Text style={[styles.subtitulo, { color: colors.textSecondary }]}>
          Ingresa con la cuenta de tu gimnasio
        </Text>
        ...
        <Pressable
          ...
          {cargando ? (
            ...
            <Text style={[styles.botonTexto, { color: colors.tintForeground }]}>Iniciar sesión</Text>
          )}
        </Pressable>
